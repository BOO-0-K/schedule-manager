...
$schedule->updateSchedule($_PICK["idx"], $_PICK["title"], $_PICK["location"], $_PICK["type"], $startTime->format("Y-m-d H:i:s"), $endTime->format("Y-m-d H:i:s"));
$schedule->deleteParticipants($_PICK["idx"]);
foreach ($participants as $participantIdx) {
    $schedule->createParticipant($_PICK["idx"], $participantIdx);
}
